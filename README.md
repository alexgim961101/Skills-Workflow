...
skills-workflow/
├── skills/                      # 12개 스킬
│   ├── ai-code-audit/           # AI 생성 코드 감사 (환각/일관성/과잉설계/완전성)
...
│   ├── performance-review/      # 성능 심층 분석 (알고리즘/DB/IO/메모리/동시성)
│   ├── rollback-strategy/       # 롤백 전략 설계
│   ├── security-review/         # OWASP 기반 보안 점검
│   └── test-strategy/           # 테스트 전략 + 품질 게이트
├── workflow/                    # 6개 워크플로우 (세션별 분리)
│   ├── plan.md                  # PLANNING 세션
│   ├── dev.md                   # EXECUTION 세션
│   ├── test.md                  # VERIFICATION 세션 1 — 테스트
│   ├── review.md                # VERIFICATION 세션 2 — 리뷰 (4-Gate)
│   ├── bugfix.md                # 버그 수정 사이클
...
2. **프로젝트 내 `.docs/` 핸드오프** — 세션 간 공유 문서는 `{PROJECT_ROOT}/.docs/`에 저장 (`implementation_plan.md`, `task.md`, `walkthrough.md`)
...
```
Session 1: /plan
  ambiguity-checklist → impact-analysis → logic-audit → rollback-strategy
  → implementation_plan.md 생성
  ↓
Session 2: /dev
  Plan 읽기 → Chunk 분할 → 구현 → 리뷰 루프
  ↓
Session 3: /test
  test-strategy → 테스트 작성/실행 → 품질 게이트
  ↓
Session 4: /review
